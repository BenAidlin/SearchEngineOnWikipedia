...
class InvertedIndex:  
  def __init__(self, docs={}):
    ...
    self.df = Counter() # key = term, value = number of documents it appeard in !
    # stores total frequency per term
    self.term_total = Counter() # key = term, value = total amount of appearances in corpus !
    # stores posting list per term while building the index (internally), 
    # otherwise too big to store in memory.
    self._posting_list = defaultdict(list) # temporary...
    ...
    # starts. 
    self.posting_locs = defaultdict(list) # key = term, value = (file_name, offset) !
	
    ###############################################################################################
	
    self.doc_len = Counter() # key = doc id, value = length of the document
	
    self.doc_vector_size = Counter() # key = doc id, value = vectorsize
	
    self.N = len(docs) # total number of documents
	################################################################################################
	
    '''
	cos_sim = (dotproduct(doc, query))/(vectorsize(doc) * vectorsize(query))
	
	tf_idf = f_(i,j)/D_j * log2(N/dfi)
	f_i,j from posting list(locs)
	D_j from doc_len
	N - total docs
	dfi from self.df
    '''
