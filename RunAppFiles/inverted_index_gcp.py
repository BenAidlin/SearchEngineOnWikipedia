...
class MultiFileWriter:
    ...
    def upload_to_gcp(self):
        ...
        blob = self.bucket.blob(f"{InvertedIndex.DIR_NAME}/{file_name}")
...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes, route_to_dir):
        b = []
        for f_name, offset in locs:
            if f_name not in self._open_files:
                blob = self.bucket.blob(f'{route_to_dir}/{f_name}')
...
class InvertedIndex:
    ...
    def posting_lists_iter(self, route_to_dir):
        """ A generator that reads one posting list from disk and yields 
            a (word:str, [(doc_id:int, tf:int), ...]) tuple.
        """
        with closing(MultiFileReader()) as reader:
            for w, locs in self.posting_locs.items():
                b = reader.read(locs[0], self.df[w] * TUPLE_SIZE, route_to_dir)
                posting_list = []
    ...
    def read_posting_list(self, w, route_to_dir):
        with closing(MultiFileReader()) as reader:
            locs = self.posting_locs[w]
            b = reader.read(locs, self.df[w] * TUPLE_SIZE, route_to_dir)
            posting_list = []
            for i in range(self.df[w]):
                doc_id = int.from_bytes(b[i * TUPLE_SIZE:i * TUPLE_SIZE + 4], 'big')
                tf = int.from_bytes(b[i * TUPLE_SIZE + 4:(i + 1) * TUPLE_SIZE], 'big')
    ...
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name):
        ...
        blob_posting_locs = bucket.blob(f"{InvertedIndex.DIR_NAME}/{bucket_id}_posting_locs.pickle")
        blob_posting_locs.upload_from_filename(f"{bucket_id}_posting_locs.pickle")


    @staticmethod
    def read_page_views(bucket_name, page_views_route):
        client = storage.Client()
        blob = client.bucket(bucket_name).blob(page_views_route)
        pick = pickle.loads(blob.download_as_string())
        return pick
